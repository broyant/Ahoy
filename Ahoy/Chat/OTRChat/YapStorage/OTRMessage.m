...
{
    [[transaction ext:AHYYapDatabaseRelationshipName] enumerateEdgesWithName:OTRMessageEdges.buddy destinationKey:uniqueBuddyId collection:[OTRBuddy collection] usingBlock:^(YapDatabaseRelationshipEdge *edge, BOOL *stop) {
...
{
    [[transaction ext:AHYYapDatabaseRelationshipName] enumerateEdgesWithName:OTRBuddyEdges.account destinationKey:uniqueAccountId collection:[AHYChatAccount collection] usingBlock:^(YapDatabaseRelationshipEdge *edge, BOOL *stop) {
...
+ (void)showLocalNotificationForMessage:(OTRMessage *)message
{
    ...
    if (state != UIApplicationStateActive && [application isRegisteredForRemoteNotifications])
    {
        dispatch_async(dispatch_get_main_queue(), ^{
            NSString * rawMessage = [message.text stringByConvertingHTMLToPlainText];
            // We are not active, so use a local notification instead
            __block OTRBuddy *localBuddy = nil;
            __block AHYChatAccount *localAccount;
            ...
            NSString *name = localBuddy.uniqueId;
...
//                localNotification.alertAction = @"rply";//REPLY_STRING;
...
+ (void)enumerateMessagesWithMessageId:(NSString *)messageId transaction:(YapDatabaseReadTransaction *)transaction usingBlock:(void (^)(OTRMessage *message,BOOL *stop))block;
{
    if ([messageId length] && block) {
        NSString *queryString = [NSString stringWithFormat:@"Where %@ = ?",AHYYapDatabseMessageIdSecondaryIndex];
        YapDatabaseQuery *query = [YapDatabaseQuery queryWithFormat:queryString,messageId];
        
        [[transaction ext:AHYYapDatabseMessageIdSecondaryIndexExtension] enumerateKeysMatchingQuery:query usingBlock:^(NSString *collection, NSString *key, BOOL *stop) {
