...
- (UIView *)mediaView {
    ...
    if (!view) {
        ...
        [[AHYDatabaseManager sharedInstance].readOnlyDatabaseConnection asyncReadWithBlock:^(YapDatabaseReadTransaction *transaction) {
            ...
            __strong typeof(weakSelf)strongSelf = weakSelf;
            [[AHYChatMediaFileManager sharedInstance] dataForItem:strongSelf buddyUniqueId:buddyUniqueId completion:^(NSData *data, NSError *error) {
                if([data length]) {
                    __strong typeof(weakSelf)strongSelf = weakSelf;
